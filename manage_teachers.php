...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_teacher'])) {
        ...
        try {
            // Check if a teacher with the same username or email already exists
            $checkStmt = $pdo->prepare("SELECT * FROM teachers WHERE username = :username OR email = :email");
            $checkStmt->execute(['username' => $username, 'email' => $email]);
            $existingTeacher = $checkStmt->fetch();

            if ($existingTeacher) {
                $error_message = "A teacher with the same username or email already exists.";
            } else {
                ...
                $success_message = "Teacher added successfully and email sent!";
            }
        } catch (Exception $e) {
            $error_message = "Failed to add teacher: {$e->getMessage()}";
        }
        ...
        try {
            ...
            $error_message = "Failed to delete teacher: {$e->getMessage()}";
        }
    }
}
...
<style>
    ...
    z-index: 1050; /* Ensure it appears above all other elements */
